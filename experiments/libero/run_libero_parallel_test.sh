...
run_libero_eval() {
    ...
    launch_task_on_pane() {
        ...
        # Each worker exposes one physical GPU. Keep the EGL ID unset during
        # robosuite import; get_libero_env selects local EGL device 0 afterward.
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "source ~/.bashrc && cd $ROOT_DIR && export EXP_NAME=$EXP_NAME && \
            ...
            env -u MUJOCO_EGL_DEVICE_ID CUDA_VISIBLE_DEVICES=$gpu_id python experiments/libero/eval_libero_single.py \
